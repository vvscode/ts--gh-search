...
const githubSearch = (str) => str
    ? fetch(`https://api.github.com/search/repositories?q=${str}`)
        .then(r => r.json())
        .then(data => data.items)
    : Promise.resolve([]);
rxjs_1.fromEvent(searchInput, 'keyup')
    .pipe(operators_1.debounceTime(500))
    .pipe(operators_1.map(ev => ev.target.value))
    .pipe(operators_1.switchMap(str => githubSearch(str)))
    .subscribe((results) => {
    outputWrapper.innerHTML = results
        .map((el) => `
    <li>
      <a href="${el.html_url}" target="_blank">${el.name} by ${el.owner.login} (${el.stargazers_count} star(s))</a>
    </li>
    `)
        .join('');
});
